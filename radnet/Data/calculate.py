...
if os.path.join(sys.path[0][:sys.path[0].rfind(os.sep)], 'radnet/Data/') not in sys.path:
    sys.path.append(os.path.join(sys.path[0][:sys.path[0].rfind(os.sep)], 'radnet/Data'))
import pyeq2
...
def fitToCurve(filterID):
	...
	#given from the initial argument and formats it so the zunzun.com code can read it. 
	...
	for dataPoint in activity:
		alpha = alpha + str(dataPoint.deltaT) + "\t" + str(dataPoint.alphaAct) + "\n"
		beta = beta + str(dataPoint.deltaT) + "\t" + str(dataPoint.betaAct) + "\n"
